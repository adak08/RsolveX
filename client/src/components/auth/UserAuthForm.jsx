...
import { Mail, Lock, Eye, EyeOff, Phone, User, ArrowRight, KeyRound, Hash } from 'lucide-react';
import api from '../../api/axios';
import { errMsg } from '../../utils/helpers';

export default function UserAuthForm({ onSuccess }) {
  const [mode, setMode] = useState('login'); // login | signup | otp-login
  const [form, setForm] = useState({
    name: '', email: '', password: '', phone: '', workspaceCode: '', otp: '',
  });
  const [showPw, setShowPw]           = useState(false);
  const [otpSent, setOtpSent]         = useState(false); // login OTP
  const [signupOtpSent, setSignupOtpSent] = useState(false); // signup OTP
  ...
  const switchMode = (m) => {
    setMode(m); setError('');
    setOtpSent(false); setSignupOtpSent(false);
    setForm(p => ({ ...p, otp: '' }));
  };
  ...
  const sendLoginOtp = async () => {
    ...
    try {
      await api.post('/api/otp/request', {
        identifier: form.email, purpose: 'login', type: 'email', userType: 'user',
      });
      setOtpSent(true);
      startTimer(setTimer);
    } catch (e) { setError(errMsg(e)); } finally { setLoading(false); }
  ...
  const handleSignup = async () => {
    setLoading(true); setError('');
    try {
      // Backend userSignup requires: name, email, password, phone, otp
  ...
  return (
    <div className="space-y-4 pt-2">
      {error && (
        <div className="p-3 rounded-xl text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/30">{error}</div>
      )}

      {/* Mode tabs */}
      <div className="flex gap-2 text-xs">
        {[['login', 'Password Login'], ['otp-login', 'OTP Login'], ['signup', 'Sign Up']].map(([m, l]) => (
          <button
            key={m}
            onClick={() => switchMode(m)}
            className={`px-3 py-1.5 rounded-lg font-medium transition-all ${mode === m ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-600' : ''}`}
        ...
        <>
          <div className="space-y-3">
            <div className="relative">
              ...
              <input className="input pl-10" placeholder="Email address" value={form.email} onChange={f('email')} type="email" />
            </div>
            <div className="relative">
        ...
        <>
          ...
          {!signupOtpSent ? (
            <button
              ...
              disabled={loading || !form.name || !form.email || !form.password || !form.phone}
